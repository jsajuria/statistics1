...
The downloaded binary packages are in
    C:\Users\phili\AppData\Local\Temp\RtmpAh67yT\downloaded_packages
\end{verbatim}
...
within the interval from negative 2.5705818 to positive
\texttt{qt(0.975,\ df\ =\ 5)}.
...
\begin{Shaded}
\begin{Highlighting}[]
\DecValTok{1} \OperatorTok{-}\StringTok{ }\KeywordTok{pt}\NormalTok{(}\FloatTok{2.9}\NormalTok{, }\DataTypeTok{df =} \DecValTok{5}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
This is the probability of getting a t value of 2.9 or larger given that
the null hypothesis is true. \texttt{pt(2.9,\ df\ =\ 5)} is the
cumulative probability of getting a t value of 2.9 or smaller (the white
area in the plot). But we want the probability of getting a value that
is as large (extreme) as 2.9 (the red area in the plot). Therefore, we
do \texttt{1\ -\ pt(2.9,\ df\ =\ 5)}.
...
\begin{Shaded}
\begin{Highlighting}[]
\DecValTok{1} \OperatorTok{-}\StringTok{ }\KeywordTok{pnorm}\NormalTok{(}\FloatTok{0.1995059}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Clearly, it was not very unlikely to find a t value of 0.1995059 under
the assumption that the null hypothesis is true. Therefore, we cannot
reject the null. The probability is 0.42 (42 percent)---highly likely.
