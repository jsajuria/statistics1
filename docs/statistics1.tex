...
\begin{verbatim}
 [1] 6.034943 5.100144 6.369319 2.479182 6.352504 2.417192 2.485653
 [8] 6.975805 1.205389 3.994078
\end{verbatim}
...
\begin{verbatim}
 [1] 5.555998 3.769431 5.403457 6.949023 2.283092 1.533248 3.062511
 [8] 2.977389 2.791993 6.111669
\end{verbatim}
...
Load \texttt{chechen.csv.} We will load it directly from the github
source.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# load texreg library for model output tables}
\KeywordTok{library}\NormalTok{(texreg)}

\CommentTok{# chechen data}
\NormalTok{a <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"https://uclspp.github.io/datasets/data/chechen.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\paragraph{Exercise 2}\label{exercise-2-5}

Run a bivariate regression model with diff as the dependent variable and
treat as the predictor.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# bivariate model}
\NormalTok{m1 <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(diff }\OperatorTok{~}\StringTok{ }\NormalTok{treat, }\DataTypeTok{data =}\NormalTok{ a)}
\KeywordTok{screenreg}\NormalTok{(m1)}
\end{Highlighting}
\end{Shaded}
...
The effect of Russian artillery shelling is significant and negative.
Therefore, it seems that indiscriminate violence in this case reduces
insurgent violence. There are many potential confounders, however, that
we will control for in the next exercise.

The interpretation on the magnitude of the effect is the following: The
dependent variable is the change in the number of insurgent attacks. The
independent variable is binary. Therefore, there was half an attack
less, on average, in villages that had been bombarded by Russian
artillery.
...
We control for the amount of attacks before the village was bombarded
(\texttt{pret}). We reason that villages that saw more insurgent
activity already, were more likely to be bombarded and might see more
insurgent activity after bombardment as well. Furhtermore, we control
for logged elevation \texttt{lelev} because attacks may be more likely
in remote mountainous terrain and it may have been harder for the
Russians to control such areas. With a similar argument we control
\texttt{groznyy} and \texttt{lpop2000}, were the argument is that urban
areas may see more insurgent actitivities and are harder to control. We
also control for \texttt{swept} to account for the possibility that
previous sweeps in the area have reduced the likelihood of subsequent
shelling and insurgent activity.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# multivariate model}
\NormalTok{m2 <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(diff }\OperatorTok{~}\StringTok{ }
\StringTok{           }\NormalTok{treat }\OperatorTok{+}
\StringTok{           }\NormalTok{pret }\OperatorTok{+}
\StringTok{           }\NormalTok{lelev }\OperatorTok{+}
\StringTok{           }\NormalTok{groznyy }\OperatorTok{+}
\StringTok{           }\NormalTok{lpop2000 }\OperatorTok{+}
\StringTok{           }\NormalTok{swept }\OperatorTok{+}
\StringTok{           }\KeywordTok{factor}\NormalTok{(disno), }
...
We increase adjusted R\^{}2 substantially. While R\^{}2 almost always
increases and never decreases, adjusted R\^{}2 might decrease. We use
the F test to test whether are added covariates are jointly significant
- whether the improvement in explanatory power of our model could be due
to chance. We reject the null hypothesis that both models do equally
well at explaining the outcome. Our larger model improves model fit.

\paragraph{Exercise 5}\label{exercise-5-7}

Check for non-linearity and if there is, solve the problem.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# create residuals variable in data set}
\NormalTok{a}\OperatorTok{$}\NormalTok{residuals <-}\StringTok{ }\NormalTok{m2}\OperatorTok{$}\NormalTok{residuals}

\CommentTok{# check residual plots with continuous variable}
\KeywordTok{plot}\NormalTok{(residuals }\OperatorTok{~}\StringTok{ }\NormalTok{pret, }\DataTypeTok{data =}\NormalTok{ a)}
\end{Highlighting}
\end{Shaded}

\includegraphics{statistics1_files/figure-latex/unnamed-chunk-457-1.pdf}

We only have one continuous variable in our model that has not already
been transformed to deal with non-linearity: the number of insurgent
attacks before shelling \texttt{pret}. The residual plot does not reveal
a non-linear relation.

\paragraph{Exercise 6}\label{exercise-6-7}

Check for heteroskedasticity and if there is solve the problem.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(lmtest) }\CommentTok{# contains BP test function}
\KeywordTok{bptest}\NormalTok{(m2) }\CommentTok{# null is constant error which we reject}
\end{Highlighting}
\end{Shaded}
...
data:  m2
BP = 78.61, df = 17, p-value = 0.0000000006764
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(sandwich) }\CommentTok{# contains sandwich estimator robust SE's}
\CommentTok{# estimate model 2 again with corrected standard errors}
\NormalTok{m3 <-}\StringTok{ }\KeywordTok{coeftest}\NormalTok{(m2, }\DataTypeTok{vcov. =} \KeywordTok{vcovHC}\NormalTok{(m2, }\DataTypeTok{type =} \StringTok{"HC1"}\NormalTok{))}

\CommentTok{# compare models}
\KeywordTok{screenreg}\NormalTok{(}\KeywordTok{list}\NormalTok{(m2,m3))}
\end{Highlighting}
\end{Shaded}
...
The result is that heteroskedasticity robust standard errors do not
change our findings. The estimated robust standard errors are actually
smaller than the ones estimated under the constant error assumption. The
effect remains significant either way but we would choose the larger
standard errors.

\paragraph{Exercise 7}\label{exercise-7-6}

Thoroughly compare the models, and interpret the outcome in substantial
as well statistical terms - do not forget to talk about model fit. What
are the implications of this?

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# compare models}
\KeywordTok{screenreg}\NormalTok{(}\KeywordTok{list}\NormalTok{(m1,m2,m3))}
\end{Highlighting}
\end{Shaded}
...
The goal of analysis was to test whether indiscriminate violence leads
to more resistance. The variable of interest is, therefore,
\texttt{treat} which indicates whether a Chechnian village was bombarded
or not. In our initial model, we found a significant and negative
effect. Thereby, we contradict the theory because the effect seems to go
the other way.

We were concerned about confounding and including a number of variables
in the model. We controled for terrain, geographic areas in the country,
urban areas, attacks before artillery bombardment, and prior sweeps. The
change in the coeffcient is large. The difference is
(\(-.52 - -0.39 = -0.13\)). That is 25\% of the original effect.
Although, the effect decreased substantially when we control for
confounders, it is still significant. The difference between villages
that were bombarded and those that were not is \(0.39\) attacks on
average.

We are not substantially interested in the control variables. According
to the model with the larger standard errors, \texttt{pret}, and
\texttt{groznyy} are also significant. Insurgent attacks were more
numerous in Groznyy. Interestingly, the more attacks there were before
the bombardment , the fewer attacks happened afterwards.

Model fit improves substantially, from \(0.03\) in our first model to
\(0.32\). Although, the second model explains the outcome better that is
not what we care about. We are interested in the effect of
indiscriminate violence. We prefer the larger model because we control
for potential confounders and our estimate less of an overestimate.

\paragraph{Exercise 8}\label{exercise-8-7}

Load who\_voted\_nazi\_in\_1932.csv.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# clear workspace }
\KeywordTok{rm}\NormalTok{(}\DataTypeTok{list=}\KeywordTok{ls}\NormalTok{())}
\NormalTok{a <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"https://uclspp.github.io/datasets/data/who_voted_nazi_in_1932.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\paragraph{Exercise 9}\label{exercise-9-6}
...
\begin{verbatim}
   shareself        shareblue       sharewhite     sharedomestic   
 ...
 Max.   :39.678   Max.   :932831   Max.   :318746   Max.   :74.09  
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{m1 <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(sharenazis }\OperatorTok{~}\StringTok{ }\NormalTok{shareblue, }\DataTypeTok{data =}\NormalTok{ a)}
\KeywordTok{screenreg}\NormalTok{(m1)}
\end{Highlighting}
\end{Shaded}
...
\paragraph{Exercise 10}\label{exercise-10-6}

Our dataset does not contain much information. We only know about the
share of diffrent groups in the district: white collar workers, blue
collar workers, self employed, domestically employed, and unemployed. We
control for these variables.

A model that would only include blue collar workers omits information
about the social make of the district that is necessarily related to the
share of blue collar workers. For, example if there are mostly white
collar workers in a district that means, there are less blue collar
workers, all else equal. If we only have blue collar in our model, we do
not know whether most other people are unemployed or white collar
workers and so on.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{m2 <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(sharenazis }\OperatorTok{~}\StringTok{ }\NormalTok{shareblue }\OperatorTok{+}\StringTok{ }\NormalTok{sharewhite }\OperatorTok{+}\StringTok{ }\NormalTok{shareself }\OperatorTok{+}\StringTok{ }\NormalTok{sharedomestic, }\DataTypeTok{data =}\NormalTok{ a)}
\end{Highlighting}
\end{Shaded}

\paragraph{Exercise 11}\label{exercise-11-5}

Can you fit a model using \texttt{shareblue}, \texttt{sharewhite},
\texttt{shareself}, \texttt{sharedomestic}, and \texttt{shareunemployed}
as predictors? Why or why not?

Such a model would suffer from perfect multicollinearity. Whichever
variable we entered last into our model would be dropped automatically.
If we add the share of the five groups, we get 100\%. Thus, if we know
the share of four gropus in a district, we necessarily know the share of
the remaining group in the district.

For example if \texttt{shareblue}=30, \texttt{shareunemployed}=15,
\texttt{sharedomestic}=25, \texttt{shareself}=20, the four groups are
together 90\% of the district. The remaining 10\% must be white collar
workers.

Therefore, adding \texttt{shareblue}, \texttt{shareunemployed},
\texttt{sharedomestic}, and \texttt{shareself} to our model already
gives us information about how many white collar workers there are in a
district. Were we to add the variable \texttt{sharewhite}, we would
enter the same information twice - leading to perfect multicollinearity.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# tests on our models }
...
data:  m1
BP = 18.578, df = 1, p-value = 0.00001631
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{bptest}\NormalTok{(m2)}
\end{Highlighting}
\end{Shaded}
...
data:  m2
BP = 95.811, df = 4, p-value < 0.00000000000000022
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# robust SE's}
\NormalTok{m1.robust <-}\StringTok{ }\KeywordTok{coeftest}\NormalTok{(m1, }\DataTypeTok{vcov. =} \KeywordTok{vcovHC}\NormalTok{(m1))}
\NormalTok{m2.robust <-}\StringTok{ }\KeywordTok{coeftest}\NormalTok{(m2, }\DataTypeTok{vcov. =} \KeywordTok{vcovHC}\NormalTok{(m2))}

\CommentTok{# compare standard errors}
\KeywordTok{screenreg}\NormalTok{(}\KeywordTok{list}\NormalTok{(m1, m1.robust, m2, m2.robust),}
          \DataTypeTok{custom.model.names =} \KeywordTok{c}\NormalTok{(}\StringTok{"m1"}\NormalTok{, }\StringTok{"m1 robust"}\NormalTok{, }\StringTok{"m2"}\NormalTok{, }\StringTok{"m2 robust"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\paragraph{Exercise 13}\label{exercise-13-3}
...
A prominent theory suggests that Nazis received a lot of their support
from blue-collar workers. In model 1, we only include the percentage of
blue collar workers in a district. The variable is insignificant but we
are likely suffering from omitted variable bias because we are omitting
other aspects of the social make-up of the district that are related to
the share blue collar workers.

We omit one category from our models to avoid perfect multicollinearity.
In model 2, it seems that blue-collar workers did support the Nazis.
More so, than white-collar workers. They received most of their support
from the self-employed, though.

\paragraph{Exercise 14}\label{exercise-14-2}

Load the Massachusetts Test Scores dataset \texttt{MA\_Schools.csv}.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{rm}\NormalTok{(}\DataTypeTok{list=}\KeywordTok{ls}\NormalTok{())}
\NormalTok{a <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"MA_Schools.csv"}\NormalTok{)}
\KeywordTok{summary}\NormalTok{(a)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{m1 <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(score8 }\OperatorTok{~}\StringTok{ }\NormalTok{english, }\DataTypeTok{data =}\NormalTok{ a)}
\KeywordTok{screenreg}\NormalTok{(m1)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{plot}\NormalTok{(score8 }\OperatorTok{~}\StringTok{ }\NormalTok{english,}
     \DataTypeTok{data =}\NormalTok{ a,}
     \DataTypeTok{pch =} \DecValTok{19}\NormalTok{,}
     \DataTypeTok{frame.plot =} \OtherTok{FALSE}\NormalTok{,}
     \DataTypeTok{col =} \StringTok{"darkgray"}\NormalTok{)}
...
\includegraphics{statistics1_files/figure-latex/unnamed-chunk-467-1.pdf}

\paragraph{Exercise 16}\label{exercise-16}

Check for correlation between the variables listed above to see if there
are other variables that should be included in the model.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{a <-}\StringTok{ }\NormalTok{a[, }\KeywordTok{c}\NormalTok{(}\StringTok{"score8"}\NormalTok{, }\StringTok{"stratio"}\NormalTok{, }\StringTok{"english"}\NormalTok{, }\StringTok{"lunch"}\NormalTok{, }\StringTok{"income"}\NormalTok{)]}
\KeywordTok{summary}\NormalTok{(a)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
     score8         stratio         english            lunch      
...
\begin{Highlighting}[]
\CommentTok{# remove NA's}
\NormalTok{a <-}\StringTok{ }\NormalTok{a[ }\OperatorTok{!}\KeywordTok{is.na}\NormalTok{(a}\OperatorTok{$}\NormalTok{score8),  ]}

\CommentTok{# check correlation}
\KeywordTok{cor}\NormalTok{(a)}
\end{Highlighting}
\end{Shaded}
...
Both \texttt{lunch} and \texttt{income} are highly correlated with the
percent of English learners and with the response variable. We add both
to our model.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{m2 <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(score8 }\OperatorTok{~}\StringTok{ }\NormalTok{english }\OperatorTok{+}\StringTok{ }\NormalTok{lunch }\OperatorTok{+}\StringTok{ }\NormalTok{income, }\DataTypeTok{data =}\NormalTok{ a)}
\end{Highlighting}
\end{Shaded}

\paragraph{Exercise 17}\label{exercise-17}

Compare the two models. Did the first model suffer from omitted variable
bias?
...
The comparison between models 1 and 2, shows that the performance of the
children is related to poverty rather than language. The two are,
however, correlated.

\paragraph{Exercise 18}\label{exercise-18}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{a}\OperatorTok{$}\NormalTok{residuals <-}\StringTok{ }\NormalTok{m2}\OperatorTok{$}\NormalTok{residuals }\CommentTok{# mistakes}
\NormalTok{a}\OperatorTok{$}\NormalTok{fitted <-}\StringTok{ }\NormalTok{m2}\OperatorTok{$}\NormalTok{fitted.values }\CommentTok{# predictions}
\KeywordTok{plot}\NormalTok{(residuals }\OperatorTok{~}\StringTok{ }\NormalTok{fitted,}
     \DataTypeTok{data =}\NormalTok{ a,}
     \DataTypeTok{pch =} \DecValTok{19}\NormalTok{,}
     \DataTypeTok{col =} \StringTok{"darkgray"}\NormalTok{,}
     \DataTypeTok{frame.plot =} \OtherTok{FALSE}\NormalTok{)}
\KeywordTok{abline}\NormalTok{(}\DataTypeTok{h=}\DecValTok{0}\NormalTok{, }\DataTypeTok{lwd =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{statistics1_files/figure-latex/unnamed-chunk-471-1.pdf}

The plot looks okay. I would not conclude that the model violates the
constant errors assumption.

\paragraph{Exercise 19}\label{exercise-19}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{bptest}\NormalTok{(m2) }\CommentTok{# we cannot reject the null -> yay :)}
\end{Highlighting}
\end{Shaded}
...
The Bresch Pagan test confirms, the constant error assumption is not
violated.

\paragraph{Exercise 20}\label{exercise-20}

Correct for heteroskedasticity (if needed) and present the results with
corrected standard errors and p-values.

We do not need to do this because the constant error assumption is not
violated.
...
\includegraphics{statistics1_files/figure-latex/unnamed-chunk-505-1.pdf}
...
\includegraphics{statistics1_files/figure-latex/unnamed-chunk-506-1.pdf}
...
a new variable called \texttt{lm\_yhats}. Those are our model
predictions or fitted values. They are predicted probabilities that the
outcome is 1 - with the downside that we can get predictions outside the
0 to 1 interval.
...
\includegraphics{statistics1_files/figure-latex/unnamed-chunk-522-1.pdf}
...
Create plots of \texttt{age} vs \texttt{VoteYes} and \texttt{LeftRight}
vs \texttt{VoteYes}. Add a regression line (from a linear probability
model) to the plots.

Here, we use the \texttt{jitter()} function to offset the observations
on the x-axis a little bit. This helps us to better see the amount of
observations that we have at each level of \texttt{age}. Furthermore,
instead of plotting dots, we plot vertical bars for the same reason.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{plot}\NormalTok{(}
  \DataTypeTok{x =} \KeywordTok{jitter}\NormalTok{(swiss}\OperatorTok{$}\NormalTok{age,}\DecValTok{1}\NormalTok{),}
  \DataTypeTok{y =}\NormalTok{ swiss}\OperatorTok{$}\NormalTok{VoteYes,}
  \DataTypeTok{pch =} \StringTok{"|"}\NormalTok{, }\DataTypeTok{bty =} \StringTok{"n"}\NormalTok{,}
  \DataTypeTok{col =} \KeywordTok{rgb}\NormalTok{(}\DataTypeTok{green =} \DecValTok{100}\NormalTok{, }\DataTypeTok{blue =} \DecValTok{100}\NormalTok{, }\DataTypeTok{red =} \DecValTok{100}\NormalTok{, }\DataTypeTok{alpha =} \DecValTok{100}\NormalTok{, }\DataTypeTok{maxColorValue =} \DecValTok{255}\NormalTok{),}
  \DataTypeTok{xlab =} \StringTok{"age"}\NormalTok{, }\DataTypeTok{ylab =} \StringTok{"vote yes (1 = yes, 0 = no)"}\NormalTok{,}
  \DataTypeTok{main =} \StringTok{"Voting Yes on Assault Rifle Ban by Age"}
\NormalTok{  )}
\KeywordTok{abline}\NormalTok{( }\KeywordTok{lm}\NormalTok{(VoteYes }\OperatorTok{~}\StringTok{ }\NormalTok{age, }\DataTypeTok{data =}\NormalTok{ swiss), }\DataTypeTok{lwd =} \DecValTok{3}\NormalTok{  )}
\end{Highlighting}
\end{Shaded}

\includegraphics{statistics1_files/figure-latex/unnamed-chunk-528-1.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{plot}\NormalTok{(}
  \DataTypeTok{x =} \KeywordTok{jitter}\NormalTok{(swiss}\OperatorTok{$}\NormalTok{LeftRight,}\DecValTok{1}\NormalTok{),}
  \DataTypeTok{y =}\NormalTok{ swiss}\OperatorTok{$}\NormalTok{VoteYes,}
  \DataTypeTok{pch =} \StringTok{"|"}\NormalTok{, }\DataTypeTok{bty =} \StringTok{"n"}\NormalTok{,}
  \DataTypeTok{col =} \KeywordTok{rgb}\NormalTok{(}\DataTypeTok{green =} \DecValTok{100}\NormalTok{, }\DataTypeTok{blue =} \DecValTok{100}\NormalTok{, }\DataTypeTok{red =} \DecValTok{100}\NormalTok{, }\DataTypeTok{alpha =} \DecValTok{100}\NormalTok{, }\DataTypeTok{maxColorValue =} \DecValTok{255}\NormalTok{),}
  \DataTypeTok{xlab =} \StringTok{"Left-Right Self Placement"}\NormalTok{, }\DataTypeTok{ylab =} \StringTok{"vote yes (1 = yes, 0 = no)"}\NormalTok{,}
  \DataTypeTok{main =} \StringTok{"Voting Yes on Assault Rifle Ban by Ideology"}
\NormalTok{  )}
\KeywordTok{abline}\NormalTok{( }\KeywordTok{lm}\NormalTok{(VoteYes }\OperatorTok{~}\StringTok{ }\NormalTok{LeftRight, }\DataTypeTok{data =}\NormalTok{ swiss), }\DataTypeTok{lwd =} \DecValTok{3}\NormalTok{  )}
\end{Highlighting}
\end{Shaded}

\includegraphics{statistics1_files/figure-latex/unnamed-chunk-529-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{m1 <-}\StringTok{ }\KeywordTok{glm}\NormalTok{(VoteYes }\OperatorTok{~}\StringTok{ }\NormalTok{age }\OperatorTok{+}\StringTok{ }\NormalTok{LeftRight, }\DataTypeTok{data =}\NormalTok{ swiss, }\DataTypeTok{family =} \KeywordTok{binomial}\NormalTok{(}\DataTypeTok{link=}\StringTok{"logit"}\NormalTok{))}
\KeywordTok{library}\NormalTok{(texreg)}
\KeywordTok{screenreg}\NormalTok{(m1)}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{exp}\NormalTok{(}\KeywordTok{coef}\NormalTok{(m1)[[}\StringTok{"LeftRight"}\NormalTok{]])}
\end{Highlighting}
\end{Shaded}
...
Conservatives are less likely to vote in favour of the ban. The odds
...
\begin{Highlighting}[]
\CommentTok{# age}
\KeywordTok{exp}\NormalTok{(}\KeywordTok{coef}\NormalTok{(m1)[[}\StringTok{"age"}\NormalTok{]])}
\end{Highlighting}
\end{Shaded}
...
Voting yes on the ban also becomes less likely with increasing age. For
each additional year the odds ratio of voting yes decrase by 2\%.

\paragraph{Exercise 5}\label{exercise-5-8}

What is the effect on the probability of a \texttt{yes} vote of moving
from a left-right self placement of 5 to a self placement of 6 for an
individual who is 44 years old?

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# predicted probability of yes-vote for left-right 5 and age 44}
\NormalTok{s1 <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(m1, }\DataTypeTok{newdata =} \KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{age =} \DecValTok{44}\NormalTok{, }\DataTypeTok{LeftRight =} \DecValTok{5}\NormalTok{), }\DataTypeTok{type =} \StringTok{"response"}\NormalTok{)}
\NormalTok{s1}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# predicted probability of yes-vote for left-right 6 and age 44}
\NormalTok{s2 <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(m1, }\DataTypeTok{newdata =} \KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{age =} \DecValTok{44}\NormalTok{, }\DataTypeTok{LeftRight =} \DecValTok{6}\NormalTok{), }\DataTypeTok{type =} \StringTok{"response"}\NormalTok{)}
\NormalTok{s2}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# first differences}
\NormalTok{s1 }\OperatorTok{-}\StringTok{ }\NormalTok{s2}
\end{Highlighting}
\end{Shaded}
...
from 53\% to 33\%. Assuming a treshold of 0.5, we would predict that the
44 years old respondent who is at 5 on the ideology scale would for the
ban. The respondent who is of the same age but at 6 on the ideology
scale would vote against the ban.

\paragraph{Exercise 6}\label{exercise-6-8}

Calculate and plot the predicted probability of voting yes across the
range of the \texttt{age} variable for individuals who have a left-right
self placement of 5. Do the same across the range of the
\texttt{LeftRight} variable for individuals with an age of 50.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# age range}
\NormalTok{s3 <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(m1, }\DataTypeTok{newdata =} \KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{age =} \KeywordTok{seq}\NormalTok{(}\KeywordTok{min}\NormalTok{(swiss}\OperatorTok{$}\NormalTok{age), }\KeywordTok{max}\NormalTok{(swiss}\OperatorTok{$}\NormalTok{age), }\DataTypeTok{length.out =} \DecValTok{100}\NormalTok{),}
                                       \DataTypeTok{LeftRight =} \DecValTok{5}\NormalTok{), }\DataTypeTok{type =} \StringTok{"response"}\NormalTok{ )}
\KeywordTok{plot}\NormalTok{(}
  \DataTypeTok{x =} \KeywordTok{jitter}\NormalTok{(swiss}\OperatorTok{$}\NormalTok{age[swiss}\OperatorTok{$}\NormalTok{LeftRight}\OperatorTok{==}\DecValTok{5}\NormalTok{],}\DecValTok{1}\NormalTok{),}
  \DataTypeTok{y =}\NormalTok{ swiss}\OperatorTok{$}\NormalTok{VoteYes[swiss}\OperatorTok{$}\NormalTok{LeftRight}\OperatorTok{==}\DecValTok{5}\NormalTok{],}
  \DataTypeTok{pch =} \StringTok{"|"}\NormalTok{, }\DataTypeTok{bty =} \StringTok{"n"}\NormalTok{,}
  \DataTypeTok{col =} \KeywordTok{rgb}\NormalTok{(}\DataTypeTok{green =} \DecValTok{100}\NormalTok{, }\DataTypeTok{blue =} \DecValTok{100}\NormalTok{, }\DataTypeTok{red =} \DecValTok{100}\NormalTok{, }\DataTypeTok{alpha =} \DecValTok{100}\NormalTok{, }\DataTypeTok{maxColorValue =} \DecValTok{255}\NormalTok{),}
  \DataTypeTok{xlab =} \StringTok{"age"}\NormalTok{, }\DataTypeTok{ylab =} \StringTok{"vote yes (1 = yes, 0 = no)"}\NormalTok{,}
  \DataTypeTok{main =} \StringTok{"Voting Yes on Assault Rifle Ban by Age"}
\NormalTok{  )}
\KeywordTok{abline}\NormalTok{(}\DataTypeTok{h=}\NormalTok{.}\DecValTok{5}\NormalTok{, }\DataTypeTok{lty =} \StringTok{"dotted"}\NormalTok{, }\DataTypeTok{lwd =} \DecValTok{3}\NormalTok{)}
\KeywordTok{text}\NormalTok{(}\DataTypeTok{x =} \DecValTok{80}\NormalTok{, }\DataTypeTok{y=}\NormalTok{ .}\DecValTok{50}\NormalTok{, }\DataTypeTok{pos =} \DecValTok{3}\NormalTok{, }\StringTok{"cutoff pi = .5"}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(}\DataTypeTok{x =} \KeywordTok{seq}\NormalTok{(}\KeywordTok{min}\NormalTok{(swiss}\OperatorTok{$}\NormalTok{age), }\KeywordTok{max}\NormalTok{(swiss}\OperatorTok{$}\NormalTok{age), }\DataTypeTok{length.out =} \DecValTok{100}\NormalTok{),}
      \DataTypeTok{y =}\NormalTok{ s3, }\DataTypeTok{lwd =} \DecValTok{3}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{statistics1_files/figure-latex/unnamed-chunk-534-1.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# max prediction}
\KeywordTok{max}\NormalTok{(s3)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# min prediction}
\KeywordTok{min}\NormalTok{(s3)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# left-right range}
\NormalTok{s4 <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(m1, }\DataTypeTok{newdata =} \KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{age =} \DecValTok{50}\NormalTok{,}
                                       \DataTypeTok{LeftRight =} \KeywordTok{seq}\NormalTok{(}\KeywordTok{min}\NormalTok{(swiss}\OperatorTok{$}\NormalTok{LeftRight, }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{), }
                                                       \KeywordTok{max}\NormalTok{(swiss}\OperatorTok{$}\NormalTok{LeftRight, }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{),}
                                                       \DataTypeTok{length.out =} \DecValTok{100}\NormalTok{)), }\DataTypeTok{type =} \StringTok{"response"}\NormalTok{ )}

\KeywordTok{plot}\NormalTok{(}
  \DataTypeTok{x=} \KeywordTok{jitter}\NormalTok{(swiss}\OperatorTok{$}\NormalTok{LeftRight[swiss}\OperatorTok{$}\NormalTok{age}\OperatorTok{==}\DecValTok{50}\NormalTok{],}\DecValTok{1}\NormalTok{),}
  \DataTypeTok{y =}\NormalTok{ swiss}\OperatorTok{$}\NormalTok{VoteYes[swiss}\OperatorTok{$}\NormalTok{age}\OperatorTok{==}\DecValTok{50}\NormalTok{],}
  \DataTypeTok{pch =} \StringTok{"|"}\NormalTok{, }\DataTypeTok{bty =} \StringTok{"n"}\NormalTok{,}
  \DataTypeTok{col =} \KeywordTok{rgb}\NormalTok{(}\DataTypeTok{green =} \DecValTok{100}\NormalTok{, }\DataTypeTok{blue =} \DecValTok{100}\NormalTok{, }\DataTypeTok{red =} \DecValTok{100}\NormalTok{, }\DataTypeTok{alpha =} \DecValTok{150}\NormalTok{, }\DataTypeTok{maxColorValue =} \DecValTok{255}\NormalTok{),}
  \DataTypeTok{xlab =} \StringTok{"Left-Right Self Placement"}\NormalTok{, }
  \DataTypeTok{ylab =} \StringTok{"Predicted Probability of Yes-Vote"}\NormalTok{,}
  \DataTypeTok{main =} \StringTok{"Effect of Ideology for Fifty Year Olds"}
\NormalTok{  )}
\KeywordTok{abline}\NormalTok{(}\DataTypeTok{h=}\NormalTok{.}\DecValTok{5}\NormalTok{, }\DataTypeTok{lty =} \StringTok{"dotted"}\NormalTok{, }\DataTypeTok{lwd =} \DecValTok{3}\NormalTok{)}
\KeywordTok{text}\NormalTok{(}\DataTypeTok{x =} \DecValTok{8}\NormalTok{, }\DataTypeTok{y=}\NormalTok{ .}\DecValTok{50}\NormalTok{, }\DataTypeTok{pos =} \DecValTok{3}\NormalTok{, }\StringTok{"cutoff pi = .5"}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(}\DataTypeTok{x =} \KeywordTok{seq}\NormalTok{(}\KeywordTok{min}\NormalTok{(swiss}\OperatorTok{$}\NormalTok{LeftRight, }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{), }
              \KeywordTok{max}\NormalTok{(swiss}\OperatorTok{$}\NormalTok{LeftRight, }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{),}
              \DataTypeTok{length.out =} \DecValTok{100}\NormalTok{),}
      \DataTypeTok{y =}\NormalTok{ s4, }\DataTypeTok{lwd =} \DecValTok{3}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{statistics1_files/figure-latex/unnamed-chunk-536-1.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# max prediction}
\KeywordTok{max}\NormalTok{(s4)}
\end{Highlighting}
\end{Shaded}
...
rapidly the more conservative the respondent, to 4\% at the extreme
right of the scale.

\paragraph{Exercise 7}\label{exercise-7-7}

We include \texttt{university}, \texttt{german}, and \texttt{urban} in
our model.

We conjecture that repsondents who are university educated are more
liberal in general and more in favour of gun control specifically.
University classes, especially those on statistics, teach that gun laws
are beneficial for society as a whole in the sense that freely available
guns do not prevent crime but are potentially harmful. Furthermore, we
control for the cultural language divide in Switzerland, where the
German speaking areas are potentially more conservative and more in
favour of liberal gun laws. We control for urbanization because we
suspect that rural areas are more status quo oriented, whereas urban
areas favour tighter regulation of guns.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{m2 <-}\StringTok{ }\KeywordTok{glm}\NormalTok{(VoteYes }\OperatorTok{~}\StringTok{ }\NormalTok{age }\OperatorTok{+}\StringTok{ }\NormalTok{LeftRight }\OperatorTok{+}\StringTok{ }\NormalTok{university }\OperatorTok{+}\StringTok{ }\NormalTok{german }\OperatorTok{+}\StringTok{ }\NormalTok{urban, }\DataTypeTok{data =}\NormalTok{ swiss, }\DataTypeTok{family =} \KeywordTok{binomial}\NormalTok{(}\DataTypeTok{link=}\StringTok{"logit"}\NormalTok{))}
\KeywordTok{screenreg}\NormalTok{(}\KeywordTok{list}\NormalTok{(m1, m2))}
\end{Highlighting}
\end{Shaded}
...
\paragraph{Exercise 8}\label{exercise-8-8}

The effect of \texttt{german} in our new model is a statistical zero. As
expected, university educated respondents are more in favour of the
...
without university education. We will keep constant \texttt{age},
\texttt{LeftRight}, and \texttt{german} and the appropriate measures of
central tendency.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# newdata with no uni degree}
\NormalTok{X1 <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}
  \DataTypeTok{age =} \KeywordTok{mean}\NormalTok{(swiss}\OperatorTok{$}\NormalTok{age),}
  \DataTypeTok{LeftRight =} \KeywordTok{mean}\NormalTok{(swiss}\OperatorTok{$}\NormalTok{LeftRight, }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{),}
  \DataTypeTok{university =} \DecValTok{0}\NormalTok{,}
  \DataTypeTok{german =} \KeywordTok{median}\NormalTok{(swiss}\OperatorTok{$}\NormalTok{german),}
  \DataTypeTok{urban =} \KeywordTok{seq}\NormalTok{(}\KeywordTok{min}\NormalTok{(swiss}\OperatorTok{$}\NormalTok{urban), }\KeywordTok{max}\NormalTok{(swiss}\OperatorTok{$}\NormalTok{urban),}\DataTypeTok{length.out =} \DecValTok{100}\NormalTok{)}
\NormalTok{)}

\CommentTok{# newdata with uni degree}
\NormalTok{X2 <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}
  \DataTypeTok{age =} \KeywordTok{mean}\NormalTok{(swiss}\OperatorTok{$}\NormalTok{age),}
  \DataTypeTok{LeftRight =} \KeywordTok{mean}\NormalTok{(swiss}\OperatorTok{$}\NormalTok{LeftRight, }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{),}
  \DataTypeTok{university =} \DecValTok{1}\NormalTok{,}
  \DataTypeTok{german =} \KeywordTok{median}\NormalTok{(swiss}\OperatorTok{$}\NormalTok{german),}
  \DataTypeTok{urban =} \KeywordTok{seq}\NormalTok{(}\KeywordTok{min}\NormalTok{(swiss}\OperatorTok{$}\NormalTok{urban), }\KeywordTok{max}\NormalTok{(swiss}\OperatorTok{$}\NormalTok{urban),}\DataTypeTok{length.out =} \DecValTok{100}\NormalTok{)}
\NormalTok{)}


\CommentTok{# no university degree}
\NormalTok{X1}\OperatorTok{$}\NormalTok{pp <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(m2, }\DataTypeTok{newdata =}\NormalTok{ X1, }\DataTypeTok{type =} \StringTok{"response"}\NormalTok{)}

\CommentTok{# got uni degree}
\NormalTok{X2}\OperatorTok{$}\NormalTok{pp <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(m2, }\DataTypeTok{newdata =}\NormalTok{ X2, }\DataTypeTok{type =} \StringTok{"response"}\NormalTok{)}


\CommentTok{# plot}
\KeywordTok{plot}\NormalTok{(}
  \DataTypeTok{x =}\NormalTok{ X1}\OperatorTok{$}\NormalTok{urban,}
  \DataTypeTok{y =}\NormalTok{ X1}\OperatorTok{$}\NormalTok{pp,}
  \DataTypeTok{type =} \StringTok{"l"}\NormalTok{,}
  \DataTypeTok{ylim =} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{1}\NormalTok{),}
  \DataTypeTok{main =} \StringTok{"Effects of Urbanisation and Higher Education on Yes-Vote"}\NormalTok{,}
  \DataTypeTok{xlab =} \StringTok{"Share of citizens in canton living in urban areas"}\NormalTok{,}
  \DataTypeTok{ylab =} \StringTok{"Predicted Probability of Yes-Vote"}\NormalTok{,}
  \DataTypeTok{lwd =} \DecValTok{3}\NormalTok{,}
  \DataTypeTok{frame.plot =} \OtherTok{FALSE}
\NormalTok{  )}
\KeywordTok{abline}\NormalTok{(}\DataTypeTok{h=}\NormalTok{.}\DecValTok{5}\NormalTok{, }\DataTypeTok{lty =} \StringTok{"dotted"}\NormalTok{, }\DataTypeTok{lwd =} \DecValTok{3}\NormalTok{)}
\KeywordTok{text}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ .}\DecValTok{8}\NormalTok{, }\DataTypeTok{y=}\NormalTok{ .}\DecValTok{50}\NormalTok{, }\DataTypeTok{pos =} \DecValTok{1}\NormalTok{, }\StringTok{"cutoff pi = .5"}\NormalTok{)}

\CommentTok{# add university education}
\KeywordTok{lines}\NormalTok{(X2}\OperatorTok{$}\NormalTok{urban, X2}\OperatorTok{$}\NormalTok{pp, }\DataTypeTok{lwd =} \DecValTok{3}\NormalTok{, }\DataTypeTok{col =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\includegraphics{statistics1_files/figure-latex/unnamed-chunk-539-1.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# no uni degree predictions}
\KeywordTok{range}\NormalTok{(X1}\OperatorTok{$}\NormalTok{pp)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# uni degree predictions}
\KeywordTok{range}\NormalTok{(X2}\OperatorTok{$}\NormalTok{pp)}
\end{Highlighting}
\end{Shaded}
...
We show the effects of urbanisation and higher education for a centrist
...
yes with 44/\% probabilty. The same respondent with higher education is
...
without higher education is 54\%. The respondent with a university
degree is predicted to vote yes with 70\% probability.

\section{Binary Dependent Variable Models
(II)}\label{binary-dependent-variable-models-ii}

\subsection{Seminar}\label{seminar-11}
...
\subsubsection{Loading Data}\label{loading-data-3}

Today, we return to ethnic voting. Instead of the Afrobarometer, we will
use a dataset from a survey that was carried out in Benin. Researchers
randomly assigned survey respondents short bibliographical passages on
the then Beninoise president Yayi Boni that included no mention of his
wife, included a mention of his wife, or included a mention of his Fon
wife. Respondents were then asked whether they were willing to vote for
Yayi Boni should an election be held and barring term limits. The goal
of the experiment was to assess whether priming respondents about the
president's Fon wife might raise support amonst wife coethics for the
president.

We start by loading the dataset \texttt{benin.csv}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{benin <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"https://raw.githubusercontent.com/UCLSPP/datasets/master/data/benin.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\texttt{sex}\strut
\end{minipage} & \begin{minipage}[t]{0.84\columnwidth}\raggedright\strut
1 if respondent is female, and 0 otherwise\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.10\columnwidth}\raggedright\strut
\texttt{age}\strut
\end{minipage} & \begin{minipage}[t]{0.84\columnwidth}\raggedright\strut
...
\end{minipage} & \begin{minipage}[t]{0.84\columnwidth}\raggedright\strut
Ethnicity of the respondent\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.10\columnwidth}\raggedright\strut
\texttt{fon}\strut
\end{minipage} & \begin{minipage}[t]{0.84\columnwidth}\raggedright\strut
1 if respondent is Fon, and 0 otherwise.\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.10\columnwidth}\raggedright\strut
\texttt{passage}\strut
\end{minipage} & \begin{minipage}[t]{0.84\columnwidth}\raggedright\strut
Control if respondent given control passage, Wife for wife passage,
FonWife for Fon wife passage\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{summary}\NormalTok{(benin)}
\end{Highlighting}
\end{Shaded}
...
We have to character variables \texttt{ethnicity} and \texttt{passage}.
Let's inspect them and turn them into factor variables and look at the
summary stats once more. Let's also turn our two numeric dummies
\texttt{sex} and \texttt{fon} into factors.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# inspect}
\KeywordTok{table}\NormalTok{(benin}\OperatorTok{$}\NormalTok{ethnicity)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{table}\NormalTok{(benin}\OperatorTok{$}\NormalTok{passage)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{benin}\OperatorTok{$}\NormalTok{ethnicity <-}\StringTok{ }\KeywordTok{as.factor}\NormalTok{(benin}\OperatorTok{$}\NormalTok{ethnicity)}
\NormalTok{benin}\OperatorTok{$}\NormalTok{passage <-}\StringTok{ }\KeywordTok{as.factor}\NormalTok{(benin}\OperatorTok{$}\NormalTok{passage)}
\NormalTok{benin}\OperatorTok{$}\NormalTok{sex <-}\StringTok{ }\KeywordTok{factor}\NormalTok{(benin}\OperatorTok{$}\NormalTok{sex, }\KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{1}\NormalTok{), }\KeywordTok{c}\NormalTok{(}\StringTok{"male"}\NormalTok{, }\StringTok{"female"}\NormalTok{))}
\NormalTok{benin}\OperatorTok{$}\NormalTok{fon <-}\StringTok{ }\KeywordTok{factor}\NormalTok{(benin}\OperatorTok{$}\NormalTok{fon, }\KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{1}\NormalTok{), }\KeywordTok{c}\NormalTok{(}\StringTok{"not fon"}\NormalTok{, }\StringTok{"fon"}\NormalTok{))}

\CommentTok{# inspect again}
\KeywordTok{table}\NormalTok{(benin}\OperatorTok{$}\NormalTok{ethnicity)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# summary stats again}
\KeywordTok{summary}\NormalTok{(benin)}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{Models with Binary Dependent Variables and
Interactions}\label{models-with-binary-dependent-variables-and-interactions}

Let's estimate a model that predicts whether a respondent would vote for
the incumbent using \texttt{passage}, \texttt{sex}, \texttt{age} as
predictors.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{m1 <-}\StringTok{ }\KeywordTok{glm}\NormalTok{(vote }\OperatorTok{~}\StringTok{ }\NormalTok{passage }\OperatorTok{+}\StringTok{ }\NormalTok{sex }\OperatorTok{+}\StringTok{ }\NormalTok{age, }\DataTypeTok{family =} \KeywordTok{binomial}\NormalTok{(}\DataTypeTok{link =} \StringTok{"logit"}\NormalTok{), }\DataTypeTok{data =}\NormalTok{ benin)}
\KeywordTok{screenreg}\NormalTok{(m1)}
\end{Highlighting}
\end{Shaded}
...
Let's interpret the table. The respondents who received the message
about Yayi Boni's wife were not more likely to say that they would vote
for the incumbent. Equally, respondents who were primed with the message
of Boni's Fon wife were not more likely to say that they intended to
vote for the incumbent. The only non-zero finding, is a gender effect.
Women are less likely to vote for the incumbent. The odds of a voting
intention for the incumbent are 43\% lower for women than for men
(\texttt{exp(-.57)}\(=0.57\)).

However, there is likely something missing from our model. Our goal was
to test whether priming respondents with the message about the Fon wife
made respondents more likely to say they would vote for Boni. We would
only reasonably expect such an effect for coethnics with the wife.
Therefore, we have to interact the treatment variable \texttt{passage}
with the coethnic indicator \texttt{fon}. We do so in model 2.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{m2 <-}\StringTok{ }\KeywordTok{glm}\NormalTok{(vote }\OperatorTok{~}\StringTok{ }\NormalTok{passage}\OperatorTok{*}\NormalTok{fon }\OperatorTok{+}\StringTok{ }\NormalTok{sex }\OperatorTok{+}\StringTok{ }\NormalTok{age, }\DataTypeTok{family =} \KeywordTok{binomial}\NormalTok{(}\DataTypeTok{link =} \StringTok{"logit"}\NormalTok{), }\DataTypeTok{data =}\NormalTok{ benin)}
\KeywordTok{screenreg}\NormalTok{(}\KeywordTok{list}\NormalTok{(m1,m2))}
\end{Highlighting}
\end{Shaded}
...
We now get significant interaction effects. We have three groups to
compare: 1) Fon respondents who got no briefing about Boni's wife. 2)
Fon respondent's who got a message about Boni's wife but not her
ethnicity. 3) Fon respondents who got a message about Boni's Fon wife.

We call the message about the wife but not her ethnicity a placebo
treatment. By giving that, we can be sure that an estimated effect is
not due to the popularity or notoriety of the wife.

For respondents in the control group, the odds ratio for those with Fon
ehnicity vs.~other ethnicity is:
\[ OR1 = exp^{\beta_{Fon}} = exp^{-2.36} = 0.09 \] The odds are reduced
by 91\%.

There is no discernable effect between Fon respondents who did not
receive a message about the wife and Fon respondents who were primed
about Boni's wife but not her ethnicity.

The odds ratio would be computed as:
\[ OR2 = exp^{\beta_{Fon} + \beta{Fon X Group}} = exp^{-2.36 + 0.19} = 0.11 \]

Finally, the interaction term between wife coethinc \texttt{fon} and
priming about the wife's ethnicity \texttt{passageFonWife:fon} is
positive and significant. For such respondents the odds ratios are
modified by \texttt{exp(-2.36\ +\ 1.76)} which is \(0.55\). Therefore,
for Fon respondents who received the treatment, the odds of voting for
Boni were reduced by only 44\%. This is evidence for the effect of
priming.

\subsubsection{Predicted Probabilities and Predictive
Power}\label{predicted-probabilities-and-predictive-power}

To assess the predictive power of our model we will check the percentage
of cases that it correctly predicted. Let's look at our dependent
variable first and find out what the naive guess is.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{mean}\NormalTok{(benin}\OperatorTok{$}\NormalTok{vote)}
\end{Highlighting}
\end{Shaded}
...
The mean is \(0.44\). That means that 44\% of the respondents said that
they would vote for Boni. This also means that more people said, that
they would not vote for him (\(100 - 44 = 56\%\)). If we had no model
and were to close our eyes and predict for a random respondent from our
sample whether she would vote for or against Boni, the prediction that
we should make is: she will not vote for Boni. This is the naive guess.
The naive guess is \(0\) - the best prediction without a statistical
model.

Now suppose, you predict \(0\) for every respondent in the sample. This
would give us a percent of correctly predicted cases of 56\%. Remember
that the mean is \(0.44\). This is the proportion of respondents who
would vote for Boni. So, if we predict that everyone does not vote for
Boni, our rate of correct predictions is 56\%. Let's get this in R.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# naive guess}
\NormalTok{nguess <-}\StringTok{ }\KeywordTok{ifelse}\NormalTok{(}\KeywordTok{mean}\NormalTok{(benin}\OperatorTok{$}\NormalTok{vote) }\OperatorTok{>}\StringTok{ }\NormalTok{.}\DecValTok{5}\NormalTok{, }\DataTypeTok{yes =} \DecValTok{1}\NormalTok{, }\DataTypeTok{no =} \DecValTok{0}\NormalTok{)}
\NormalTok{nguess}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# percent correctly predicted using the naive guess}
\NormalTok{pcp.naive <-}\StringTok{ }\KeywordTok{ifelse}\NormalTok{(}\KeywordTok{mean}\NormalTok{(benin}\OperatorTok{$}\NormalTok{vote) }\OperatorTok{>}\StringTok{ }\FloatTok{0.5}\NormalTok{, }\DataTypeTok{yes =} \KeywordTok{mean}\NormalTok{(benin}\OperatorTok{$}\NormalTok{vote), }\DataTypeTok{no =} \DecValTok{1} \OperatorTok{-}\StringTok{ }\KeywordTok{mean}\NormalTok{(benin}\OperatorTok{$}\NormalTok{vote))}
\NormalTok{pcp.naive}
\end{Highlighting}
\end{Shaded}
...
We see that the naive guess is indeed 56\%. Our statistical model that
we estimated earlier must do better than this, otherwise our model will
be useless. You can think of this like a linear model that does not win
the F-test against the empty model. Such a model makes no contribution.

We will now estimate predicted probabilities, expected values and
estimate our rate of correctly predicted cases.

Step 1: Predicted probabilities. We already know how to do this. We use
the \texttt{predict()} function and set \texttt{type\ =\ "response"}. We
attach the predicted probabilities for every respondent to the original
dataset.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# predicted probabilities}
\NormalTok{benin}\OperatorTok{$}\NormalTok{pps <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(m2, }\DataTypeTok{type =} \StringTok{"response"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

Step 2: Expected values. Now, that we got our predicted probabilities of
voting for Boni, we will assign everyone in our dataset who has a
predicted probability greater than \(0.5\) with an expected value of 1
and everyone else get's a \(0\).

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{benin}\OperatorTok{$}\NormalTok{evs <-}\StringTok{ }\KeywordTok{ifelse}\NormalTok{(benin}\OperatorTok{$}\NormalTok{pps }\OperatorTok{>}\StringTok{ }\FloatTok{0.5}\NormalTok{, }\DataTypeTok{yes =} \DecValTok{1}\NormalTok{, }\DataTypeTok{no =} \DecValTok{0}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

Step 3: Compute percentage correctly predicted. For every respondent in
our dataset, we have made a prediction based on our model \texttt{m2}.
We will compare our predictions to the actual outcomes now in a
cross-table (sometimes called confusion matrix).

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{confusion <-}\StringTok{ }\KeywordTok{table}\NormalTok{(}\DataTypeTok{actual =}\NormalTok{ benin}\OperatorTok{$}\NormalTok{vote, }\DataTypeTok{expected.value =}\NormalTok{ benin}\OperatorTok{$}\NormalTok{evs)}
\NormalTok{confusion}
\end{Highlighting}
\end{Shaded}
...
The observations on the diagonal are our correct predictions where a
predicted vote intention against Boni corresponds to an actual vote
intention against Boni and a predicted vote intention for Boni
corresponds to an actual vote intention to Boni.

For 55 respondents, we predict that they do not want to vote for Boni
but they respond that they do want to vote for him. These are what we
call false negatives.

For 60 respondents, we predict that they would want to vote for Boni but
when asked they said that they would not want to vote for him. We call
these mistakes false positives.

For our purposes, false negatives are equally bad as false positives. We
do want to make as few mistakes as possible but we do not care about the
kind of mistake that we make. There are other applications where the
kind of mistake matters (e.g.~cancer tests). By varying the threshold
from 0.5 to some other threshold value, we would change the amount of
false negative and false positive mistakes that we make but we cannot
get a lower overall rate of mistakes by changing the threshold (0.5 is
optimal in that sense).

To compute the percentage of correctly classified cases, we now divide
the elements on the diagonal of the confusion matrix by all respondents.
We use the \texttt{diag()} function and the \texttt{sum()} function to
do this. The former takes the diagonal elements of the table and the
latter takes a sum.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{sum}\NormalTok{(}\KeywordTok{diag}\NormalTok{(confusion)) }\OperatorTok{/}\StringTok{ }\KeywordTok{sum}\NormalTok{(confusion)}
\end{Highlighting}
\end{Shaded}
...
We correctly classify 67\% of the respondents into Boni supporters and
opponents. We thereby improve upon the naive guess which correctly
classifies 56\% of the respondents. Here, we compared against a baseline
of no model. When we try to decide between models, we can use the same
approach.

\subsubsection{Joint hypothesis testing}\label{joint-hypothesis-testing}

Our first model \texttt{m1} included \texttt{passageFonWife},
\texttt{passageWife}, \texttt{sex}, and \texttt{age} as predictors. In
our second model, we also included \texttt{fon} as well as interactions
between \texttt{passageFonWife} and \texttt{fon}, and also
\texttt{passageWife} and \texttt{fon}. We estimated three additional
parameters in model \texttt{m2} compared to model \texttt{m1}. Let's
...
As you can see, the log-likelihood in \texttt{m2} is larger than the
log-likelihood in \texttt{m1}. That means that the same is true for the
...
In R, you need to load the \texttt{libary(lmtest)} to carry out the
test. The actual test is done by using the \texttt{lrtest()} function
which stands for likelihood-ratio test. We will do this now. The null
hypothesis is that \texttt{m2} is not an improvement on \texttt{m1} but
that they are similar models, i.e.~both explain the world equally well.
We can reject that null if the p value is small. Here, we require it to
be less than \(0.05\).

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{lrtest}\NormalTok{(m1, m2)}
\end{Highlighting}
\end{Shaded}
...
\paragraph{Predicted Probabilities and Confidence
intervals}\label{predicted-probabilities-and-confidence-intervals}

We have constructed confidence intervals for our estimates before. Here,
we want to quantify the uncertainty of our predictions. Imagine, we
predict that a respondent will vote for Boni with 85\% probability. This
is what we call a point estimate, a prediction, of the outcome.
Beacause, we predict this quantity from our model and our model
parameters follow distributions, so does the prediction of the outcome.

With a confidence interval of the prediction, we can show how precise
our prediction is. Let's say, we predict that the respondent will vote
for Boni with 80\% to 90\% probability. That would be a more precise
statement than if we were to predict that the respondent votes for Boni
with 60\% to 100\% probability.

The precision of the outcome depends on precision of our estimated
parameters and their covariance. We will predict the probability of
voting for Boni for the treatment group and Fon coethnic respondent and
control groups.

To illustrate the uncertainty of our estimates we will use the
\texttt{predict()} function and set the argument
\texttt{se.fit\ =\ TRUE} which will return standard errors alongside
predictions. We will use the standard error to construct our confidence
intervals.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# treatment group covariates}
\NormalTok{Xt <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}
  \DataTypeTok{passage =} \StringTok{"FonWife"}\NormalTok{, }
  \DataTypeTok{fon =} \StringTok{"fon"}\NormalTok{, }
  \DataTypeTok{sex =} \StringTok{"male"}\NormalTok{,}
  \DataTypeTok{age =} \KeywordTok{mean}\NormalTok{(benin}\OperatorTok{$}\NormalTok{age)}
\NormalTok{  )}

\CommentTok{# control group 1 covariates }
\NormalTok{Xc <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}
  \DataTypeTok{passage =} \StringTok{"Wife"}\NormalTok{,}
  \DataTypeTok{fon =} \StringTok{"fon"}\NormalTok{,}
  \DataTypeTok{sex =} \StringTok{"male"}\NormalTok{,}
  \DataTypeTok{age =} \KeywordTok{mean}\NormalTok{(benin}\OperatorTok{$}\NormalTok{age)}
\NormalTok{)}

\CommentTok{# predictions treatment}
\NormalTok{treat.prediction <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(m2, }\DataTypeTok{newdata =}\NormalTok{ Xt, }\DataTypeTok{type =} \StringTok{"response"}\NormalTok{, }\DataTypeTok{se.fit =} \OtherTok{TRUE}\NormalTok{)}
\CommentTok{# upper bound }
\NormalTok{treat.upper <-}\StringTok{ }\NormalTok{treat.prediction}\OperatorTok{$}\NormalTok{fit }\OperatorTok{+}\StringTok{ }\FloatTok{1.96} \OperatorTok{*}\StringTok{ }\NormalTok{treat.prediction}\OperatorTok{$}\NormalTok{se.fit}
\NormalTok{treat.upper}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{treat.lower <-}\StringTok{ }\NormalTok{treat.prediction}\OperatorTok{$}\NormalTok{fit }\OperatorTok{-}\StringTok{ }\FloatTok{1.96} \OperatorTok{*}\StringTok{ }\NormalTok{treat.prediction}\OperatorTok{$}\NormalTok{se.fit}
\NormalTok{treat.lower}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{treat.point <-}\StringTok{ }\NormalTok{treat.prediction}\OperatorTok{$}\NormalTok{fit}
\NormalTok{treat.point}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# predictions control (message about wife but no coethnic)}
\NormalTok{pps.control <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(m2, }\DataTypeTok{newdata =}\NormalTok{ Xc, }\DataTypeTok{type =} \StringTok{"response"}\NormalTok{, }\DataTypeTok{se.fit =} \OtherTok{TRUE}\NormalTok{)}
\CommentTok{# upper bound}
\NormalTok{control.upper <-}\StringTok{ }\NormalTok{pps.control}\OperatorTok{$}\NormalTok{fit }\OperatorTok{+}\StringTok{ }\FloatTok{1.96} \OperatorTok{*}\StringTok{ }\NormalTok{pps.control}\OperatorTok{$}\NormalTok{se.fit}
\NormalTok{control.upper}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{# lower bound}
\NormalTok{control.lower <-}\StringTok{ }\NormalTok{pps.control}\OperatorTok{$}\NormalTok{fit }\OperatorTok{-}\StringTok{ }\FloatTok{1.96} \OperatorTok{*}\StringTok{ }\NormalTok{pps.control}\OperatorTok{$}\NormalTok{se.fit}
\NormalTok{control.lower}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{# point estimate}
\NormalTok{control.point <-}\StringTok{ }\NormalTok{pps.control}\OperatorTok{$}\NormalTok{fit}
\NormalTok{control.point}
\end{Highlighting}
\end{Shaded}
...
We predict that the Fon respondent who got the treatment message, votes
for Boni with 47\%. Our estimate is the range 32\% to 62\%. For the Fon
respondent who received the control treatment, we predict that she votes
for Boni with probability 18\%. The estimate is in the range from 5\% to
31\%.
...
\item
  ...
  \texttt{non\_western\_foreingners.RData}.
\item
  Build a model that predicts \texttt{over.estimate} to find
  determinants of over-estimating the level of migration. Justify your
  choice of predictors and interpret the model output.
\item
  What is the naive guess of \texttt{over.estimate}?
\item
  What is the percentage of correctly predicted cases using the naive
  guess?
\item
  What is the percent of correctly predicted cases of your model and how
  does it compare to the naive guess?
\item
  Add predictors to your model and justify the choice.
\item
  Are the added predictors jointly significant?
\item
  What is the new rate of correctly predicted cases? Is it better than
  the first model and how does it compare to the naive guess?
\item
  Vary the treshold for generating expected values and compute the
  percent correctly predicted cases again. How does it change?
\item
  Vary a continuous predictor in a model and plot predicted
  probabilities over its range and include uncertainty estimates in the
  plot.
\end{enumerate}
