...
\texttt{c()} function. \texttt{c()} stands for collect or concatenate.
...
\texttt{rep()}, replicates elements of vector. Its arguments are the
item \texttt{x} to be replicated and the number of times that \texttt{x}
should be replicated. Below, we create the variable education with the
frequency of education level indicated above. Note that the arguments
\texttt{x} and \texttt{times} do not have to be written out.
...
The mode is the appropriate measure of central tendency if level of
measurement is nominal. Nominal means that there is no ordering implicit
in the values that a variable takes, even if these values are numbers.
We create a dataset of 1000 (fake) voters in the United Kingdom who each
express their preference on remaining or leaving the European Union. The
options are leave or stay. Leaving is not greater than staying and or
vice versa (even though we all order the two options normatively).
...
Our goal is to understand by how much a realisation of some variable
differs from its mean on average. Let's assume that our variable is
income in the UK. Let's assume that its mean is 35 000 per year. Now if
the average deviation from that value were 5 000, this would mean that
if we ask 100 people at random about their income, they would on average
differ by 5 000 from 35 000. Suppose that the average income in France
is also 35 000 per year but the average deviation is 10 000 instead.
This of course implies that income is more equally distributed in the UK
than in France. Dispersion is important to describe data as this example
illustrates. Although, income in our hypothetical example is the same in
France and the UK, both societies are qualitatively different from one
another.
...
next to no effect if n is large but it its effect increases the smaller
n. The smaller n, the larger the sample variance. The intuition is that
in smaller samples we are less certain that our sample reflects the
...
Take a minute to think your way through the formula to understand what
is happening. In the numerator, we subtract the mean of x from some
realisation of x. The table below takes our homework grade example.
...
To compute the variance, we are taking the deviations from the mean for
each observation. We are squaring the deviations and then dividing them
by the number of observations. You can see that our first grade (80) is
below the mean (83.9090909). The sum is, thus, negative. Our second
grade (90) is above the mean, so that the sum is positive. We add up the
deviations and we each deviation should contribute to the total sum of
deviations. Therefore, we must avoid negative deviations. Our deviations
should be 0 or larger. The square of each deviation is always positive
as you can see in the fourth column. Now we divide the sum of the
squared deviations by the number of deviations to get a sense of the
average. The variance is somewhat awkward because its units are squared
deviations.

The variance gives us average squared deviations from the mean. We take
the variance in R by using the
...
However, no education is not a common value. One person in our sample
did not have education. The interquartile range is the range from the
25th to the 75th percentiles, i.e., it contains the central 50 percent
of the distribution.
...
quantile \href{http://bit.ly/R_quantile}{\texttt{quantile()}} in R to
get percentiles. The function takes two arguments. First, \texttt{x} is
the data vector. Second, \texttt{probs} is the percentile.
