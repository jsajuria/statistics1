...
Let's take a look at a macroeconomic dataset called `longley`. The `longley` dataset is provided as a data frame of `r ncol(longley)` variables and `r nrow(longley)` observations.

```{r eval = FALSE}
help(longley)
```

The help screen describes each of the `r ncol(longley)` variables. Now let's see what's in the longley dataset.
...
```{r}
plot(x = longley$Year, y = longley$Employed, type = "l")
```
...
```{r class.source = "collapsible"}
plot(x = longley$Year, y = longley$Employed, type = "b")
```
...
|Ordered   |proportion in each category|
...
|Observation|Variable|deviation from the mean| squared deviation from mean|
|--------------|-----------------------|--------------|----------------|
