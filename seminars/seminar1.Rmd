...
We calculate the average grade on our eleven homework assignments in statistics 1. We create our vector of 11 (fake) grades first using the `c()` function. `c()` stands for collect or concatenate.
...
The median is the appropriate measure of central tendency for ordinal variables. Ordinal means that there is a rank ordering but not equally spaced intervals between values of the variable. Education is a common example. In eduacation, more education is better. But the difference between primary school and secondary school is not the same as the difference between secondary school and an undergraduate degree.
...
We introduce a new function to create a vector. The function `rep()`, replicates elements of vector. Its arguments are the item `x` to be replicated and the number of times that `x` should be replicated. Below, we create the variable education with the frequency of education level indicated above. Note that the argumens `x` and `times` do not have to be written out.
...
Take a minute to think your way through the formula to understand what is happening. In the numerator, we subtract the mean of x from some realisation of x. The table below takes our homework grade example.
...
|$\ n-1$ |  |  | `r sum((hw.grades - mean(hw.grades))^2)/(length(hw.grades)-1)`|
|$\sqrt{}$ |  |  | `r sqrt(sum((hw.grades - mean(hw.grades))^2)/(length(hw.grades)-1))`|

Following the formula, we are taking the deviations from the mean for each observation. We are squaring the deviations and then dividing them by the number of observations. You can see that our first grade (`r hw.grades[1]`) is below the mean (`r mean(hw.grades)`). The sum is, thus, negative. Our second grade (`r hw.grades[2]`) is above the mean, so that the sum is positive. We add up the deviations and we each deviation should contribute to the total sum of deviations. Therefore, we must avoid negative deviations. Our deviations should be 0 or larger. The square of each deviation is always positive as you can see in the fourth column. Now we divide the sum of the squared deviations by the number of deviations to get a sense of the average. The variance is somewhat akward because its units are squared deviations. 

The variance gives us average squared deviations from the mean. We take the variance in R by using the [`var()`](http://bit.ly/R_var) function. By default `var()` takes the sample variance.
...
The proper measure of dispersion of an ordinal variable is the range or the interquartile range. The interquartile range is usually the preferred measure because the range is stongly affected by outlying cases.
...
Our data ranges from no education all the way to those with a doctorate. However, no education is not a common value. One person in our sample did not have education. The interquartile range is the range rom the 25th to the 75th percentiles, i.e., it contains the central 50 percent of the distribution.

The 25th percentile is the value of education that 25 percent or fewer poeple have (when we order education from lowest to highest). We use the quantile [`quantile()`](http://bit.ly/R_quantile) in R to get percentiles. The function takes two arguments. First, `x` is the data vector. Second, `probs` is the percentile.
...
To describe the distriubution of our nominal variable, support for remaining in the European Union, we use the proportions in each category.
...
To get the proportions in each category, we devide the values in the table, i.e., 509 and 491, by the sum of the table, i.e., 1000.
...
1. Download this [cheat-sheet](https://www.rstudio.com/wp-content/uploads/2016/06/r-cheat-sheet.pdf) and go over it. You won't understand most of it right a away. But it will become a useful resource. Look at it often.
1. Calculate the square root of 1369 using the `sqrt()` function.
1. Square the number 13 using the `^` operator.
1. What is the result of summing all numbers from 1 to 100?
...
1. Describe Berlin income using the apropriate measures of central tendency and dispersion.
1. Compute the average deviation without using the `sd()` function.
...
1. Take the most recent poll and describe what you see in terms of central tendency and dispersion.
1. Save your script, which should now include the answers to all the exercises.
1. Source your script, i.e. run the entire script without error message. Clean your script if you get error messages.