...
In today's seminar, we learn working with data frames. We will create our own dataset, we subset dataset (access elements, rows and variables). We load our first dataset into R. We also visualise data using the `plot()` function. Finally, we estimate a treatment effect in R---our first inference.
...
Last week we have already worked with vectors. We created a sequence for example. This week, we have a look at element-wise operations and we see how to access elements within a vector.
...
To access all elements except the last element we use the `-` operator.
...
If we want to see to specific non-adjacent elements, we can use the collect function `c()`.
```{r}
...
We load the dataset with the `load()` function like. 
...
The non-western foreingers data is about the subjective perception of immigrants from non-western countries. The perception of immigrants from a context that is not similar to the own is often used as a proxy for racism. Whether this is a fair measure or not is debatable but let's examine the data from a survey carried our in Britain. 
...
It is plausible that perception of immigration from Non-Western countries is related to party affiliation. In our dataset, we have a few vaiables that are related to party affiliation. We can use square brackets to subset our data such that we produce a boxplot only for members of the conservative party. We have a look at the variable *Cons* using the table function.
...
We would now like to compare this with the Labour party. We could subset the data just like we did for the Conservative Party. We will do this. We also want to plot the two plots next to each other. We can do this with the `par()` and the `mfrow` argument. This will spilt the plot window into rows and columns. We want to columns to plot two boxplots next to each other. 
...
Let's suppose, associating with right-wing parties causes people to over-estimate the number of non-western foreigners in the Britain. Our treatment variable is whether one assoicates with the UK Independence Party. It is 1 if that is the case and 0 otherwise. Let's inspect the variable *Ukip*.
